...
found_reflection = 0
found_reflection_chg = 0
for section_raw in list_final: #Going through each section
    print('='*30)
    section_numpy = np.array(section_raw, ndmin=2)
    for (i_chg,j_chg), values in np.ndenumerate(section_numpy):
        print("i: ", i_chg)
        print("j: ", j_chg)
        section_copy = np.copy(section_numpy)
        print(section_copy[i_chg][j_chg])
        if found_reflection_chg == 1:
            found_reflection_chg = 0
            break
        symbol = section_copy[i_chg][j_chg]
        print("This is symbol: ", symbol)
        if symbol == '.':
            print('here1')
            section_copy[i_chg][j_chg] = '#'
        else:
            print('here2')
            section_copy[i_chg][j_chg] = '.'
        section = np.copy(section_copy)
        ...
        for i, row in enumerate(section[:-1]):
            if np.all(row == section[i+1]):#Find first reflection point
                ...
                while U_idx>0 and B_idx < len(section)-1 and np.all(section[U_idx] == section[B_idx]):
                    ...
                    B_idx += 1
                    end_chk = 1
                if np.all(section[U_idx] == section[B_idx]) and U_idx <= i_chg and B_idx >= i_chg:
                    print(section)
        ...
        if found_reflection == 0: #No rows found            
            for i, col in enumerate(section.T[:-1]):
                if np.all(col == section.T[i+1]):#Find reflection point
                    ...
                    if np.all(section.T[L_idx] == section.T[R_idx]) and L_idx <= j_chg and R_idx >= j_chg:
                        print("Reflection found!")
                        found_reflection = 1 
                        end_chk = 1
                if found_reflection == 1:
